...
"""
Train the four machine learning models for different values of k for k-anonymity.
"""


def train_models(data: str, test_split: int = 42):
    df = pd.read_csv(data)
    ...
    X = df[
        ["age", "education", "relationship", "occupation", "sex", "native-country"]
    ].values
    y = df["salary-class"].values

    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=0.25, random_state=test_split, stratify=y
    )
